/* this step creates all appendices, including a pdf document incorporating tables and figures*/ 

set more off
qui do par.do


use ${dirdtain}AESI_code_counts_labelled_FOR_APPENDIX,clear
export excel using ${dirtab}APPENDIX_CODE_COUNTS,replace first(var)


use ${dirdtain}IR_covid_calendartime_vax_cohort_labelled,clear
isid COVID ordDAP week_str ordvac dose ageband
order COVID ordDAP week_str ordvac dose ageband IR lb ub  year month
keep COVID ordDAP week_str ordvac dose ageband IR lb ub  year month
sort COVID ordDAP week_str ordvac dose ageband
export excel using ${dirtab}APPENDIX_IR_covid_calendartime,replace first(var)

use ${dirdta}IR_fup_weeks_cumulative_and_BGR,clear
keep system_num AESI_num ordDAP ordvac dose cumulative_week_fup Age IR LL UL IR_BGR lb_BGR ub_BGR AESI
order system_num AESI_num ordvac dose ordDAP Age cumulative_week_fup IR LL UL IR_BGR lb_BGR ub_BGR
sort system_num AESI_num ordvac dose ordDAP Age cumulative_week_fup
export excel using ${dirtab}APPENDIX_IR_AESI_fup_weeks_cumulative_and_BGR,replace first(var)


global namedoc "annex_pdf"
...
global titolo_documento "Annex to updated November 2021 deliverable: monitoring"
...
file write `rel' `"\section{Incident cases in 2020 and 2021}"' _n _n
...
foreach AESI in `AESIlist'{
	file write `rel' `"\section{${name`AESI'}}"' _n _n
