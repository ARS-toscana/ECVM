...
#change ConcePTION in ConcePTION
...
for (i in 1:length(files)) {
  if (str_detect(files[i],"^SURVEY_OB")) { ConcePTION_CDM_EAV_tables_retrieve[[files[i]]]<-list("so_source_table", "so_source_column")
  }
  else{if (str_detect(files[i],"^MEDICAL_OB")){ ConcePTION_CDM_EAV_tables_retrieve[[files[i]]]<-list("mo_source_table", "mo_source_column")
  }
  }
}
