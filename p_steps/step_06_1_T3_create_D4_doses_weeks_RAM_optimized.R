...
person_id_table <- unique(cohort_to_vaxweeks[, .(person_id)])[, person_id_code := seq_len(.N)]
cohort_to_vaxweeks <- join_and_replace(cohort_to_vaxweeks, person_id_table, c("person_id", "person_id"), "person_id_code")
rm(person_id_table)
birth_cohort_table <- unique(cohort_to_vaxweeks[, .(Birthcohort_persons)])[, Birthcohort_persons_code := seq_len(.N)]
cohort_to_vaxweeks <- join_and_replace(cohort_to_vaxweeks, birth_cohort_table,
                                       c("Birthcohort_persons", "Birthcohort_persons"), "Birthcohort_persons_code")
rm(birth_cohort_table)
...
start_date_table <- unique(cohort_to_vaxweeks[, .(start_date_of_period)])[, start_date_of_period_code := seq_len(.N)]
cohort_to_vaxweeks <- join_and_replace(cohort_to_vaxweeks, start_date_table,
                                       c("start_date_of_period", "start_date_of_period"), "start_date_of_period_code")
...
setnames(start_date_table, c("start_date_of_period_code", "start_date_of_period"), c("start_date_of_period", "start_date_of_period_code"))
cohort_to_vaxweeks <- join_and_replace(cohort_to_vaxweeks, start_date_table,
                                       c("start_date_of_period", "start_date_of_period"), "start_date_of_period_code")
