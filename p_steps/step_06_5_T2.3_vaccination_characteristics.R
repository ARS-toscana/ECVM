...
D3_Vaccin_cohort_cov <- vector(mode = 'list')
for (subpop in subpopulations_non_empty) {
  ...
  if (this_datasource_has_subpopulations == TRUE){ 
    D3_Vaccin_cohort_cov[[subpop]] <- study_population_cov
  }else{
    D3_Vaccin_cohort_cov <- study_population_cov
  }
} 
...
rm(D3_study_population_covariates,D3_Vaccin_cohort_cov)
